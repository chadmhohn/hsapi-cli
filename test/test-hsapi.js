...
async function main() {
  const { server, requests, routeCounts, baseUrl } = await startServer();
  const configPath = writeTempConfig(baseUrl);
  const baseEnv = {
  ...
  const uploadFile = path.join(importDir, 'logo.txt');
  const importRequestPath = path.join(importDir, 'import-request.json');
  ...
  try {
    ...
    const developerOnlyConfig = writeTempConfig(baseUrl, {
      tokenEnv: null,
      auth: {
