...
Use the official HubSpot CLI for project workflows unless a future `hsapi project ...` bridge explicitly delegates to that CLI:
...
HubSpot Projects are local developer-tooling workflows, not ordinary portal-bearer CMS REST calls. Their auth is owned by the official `hs` CLI and its account or personal-access-key setup.
...
- Any future `hsapi project ...` bridge must make delegation to `hs project ...` obvious, report auth/account provenance without secrets, and keep mutating or deploy-style commands behind an explicit safety gate.
...
4. `hs project list --account <account>`
